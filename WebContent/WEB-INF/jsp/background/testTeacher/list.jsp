...
<body>
<form id="fenye" name="fenye" action="<%=request.getContextPath()%>/background/testTeacher/find.html" method="post">
                    ...
                    <td>
	                    <div class="box_bottom pb5 pt5 pr10" style="border-top:0px solid #dadada;">
			              <div class="search_bar_btn" style="text-align:right;">
	                         ...
	                         <sec:authorize ifAnyGranted="ROLE_test_teacher_add">
	                            <input type="button" name="button" class="btn btn82 btn_add" value="新增" onclick="GOTO('<%=request.getContextPath()%>/background/testTeacher/addUI.html')">
	                         </sec:authorize>
	                         <sec:authorize ifAnyGranted="ROLE_test_teacher_delete">
            ...
            <div class="box span10 oh">
              <table class="list_table" width="100%" cellspacing="1" onmouseover="changeto()"  onmouseout="changeback()">
			          ...
			          <c:forEach var="key" items="pageView.data" >
						...
						<td align="center" ><a href="<%=request.getContextPath()%>/background/testTeacher/findById.html?objId=${key.id}&type=0">${key.id}</a></td>
						<td align="center" >${key.infoDesc}</td>
						<td align="center" >${key.name}</td>
						<td align="center" >${key.mobile}</td>
						<td align="center" >${key.gender}</td>
						<td align="center" >${key.age}</td>
						<td align="center" >${key.major}</td>
						<td align="center" >${key.score}</td>
						<td align="center" >
						
						<sec:authorize ifAnyGranted="ROLE_test_teacher_info">
							<img src="<%=request.getContextPath()%>/images/admin.gif" width="10" height="10" />
							<a href="<%=request.getContextPath()%>/background/testTeacher/findById.html?objId=${key.id}&type=0">
							详细信息</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_test_teacher_edit">
							<img src="<%=request.getContextPath()%>/images/edt.gif" width="12" height="12" />
							<a href="<%=request.getContextPath()%>/background/testTeacher/findById.html?objId=${key.id}&type=1">编辑</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_test_teacher_delete">
							<img src="<%=request.getContextPath()%>/images/del.gif" width="16" height="16" />
							<a href="javascript:void(0);" onclick="deleteId('<%=request.getContextPath()%>/background/testTeacher/deleteById.html','${key.id}')">删除</a>
						</sec:authorize>
