...
public class LoginSessionUtils extends MultiCacheStrutsBaseDaoImpl {
	...
	@PostConstruct
	public void start() {
		redisSession = factory.getRedisSessionFactory().getRedisSession();
	...
	private static void saveOrUpdate(String localSessionId, UserSession userSession) {
		redisSession.setData(LOCAL_USERSESSION_REDIS_HEAD + localSessionId, JsonConvertUtils.toJson(userSession),SESSION_TIMEOUT_SECONDS);
	}
